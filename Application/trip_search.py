...
#Interrogo per ottenere il codice della stazione di PARTENZA
    ...
    if CodiceStazionePartenza=='':
        ...
        NomeCompletoStazionePartenza=CodiceStazionePartenza[:CodiceStazionePartenza.index('|')]
        if '\n' in CodiceStazionePartenza:
            CodiceStazionePartenza=CodiceStazionePartenza[CodiceStazionePartenza.index('|')+2:CodiceStazionePartenza.index('\n')]
        else:
            CodiceStazionePartenza=CodiceStazionePartenza[CodiceStazionePartenza.index('|')+2:]
...
#Interrogo per ottenere il codice della stazione di ARRIVO
    ...
    if CodiceStazioneArrivo=='':
        ...
        NomeCompletoStazioneArrivo=CodiceStazioneArrivo[:CodiceStazioneArrivo.index('|')]
        if '\n' in CodiceStazioneArrivo:
            CodiceStazioneArrivo=CodiceStazioneArrivo[CodiceStazioneArrivo.index('|')+2:CodiceStazioneArrivo.index('\n')]
        else:
            CodiceStazioneArrivo=CodiceStazioneArrivo[CodiceStazioneArrivo.index('|')+2:]
...
#Interrogo con codici delle stazioni di partenza e ritorno e data e ora CODICEPARTENZA/CODICEARRIVO/AAA-MM-GGTHH:MM:SS occhio alla T che separa data e ora
    ...
    try:
        for sol in range (0,2): #ciclo che fa visualizzare le prime 2 opzioni
            ...
            departure=[]
            arrival=[]
            for cont in range(0,len(parsed_json['soluzioni'][sol]['vehicles'])): #ciclo che crea il messaggio per quando la soluzione ha piu treni
                ...
                trains.append(parsed_json['soluzioni'][sol]['vehicles'][cont]['numeroTreno'])
                departure.append(parsed_json['soluzioni'][sol]['vehicles'][cont]['origine'])
                arrival.append(parsed_json['soluzioni'][sol]['vehicles'][cont]['destinazione'])
                ...
                risp+=(':train2: Treno %s %s\n:arrow_right: <b>%s</b> da %s\n:arrow_left: <b>%s</b> a %s\n\n' %(parsed_json['soluzioni'][sol]['vehicles'][cont]['categoriaDescrizione'], parsed_json['soluzioni'][sol]['vehicles'][cont]['numeroTreno'], departure_time, parsed_json['soluzioni'][sol]['vehicles'][cont]['origine'], arrival_time, parsed_json['soluzioni'][sol]['vehicles'][cont]['destinazione']))
            response= risp=':white_check_mark:<b>Soluzione %s</b>\n\n:calendar: %s-%s-%s\n:clock10:Durata <b>%s</b>\n\n%s' %(sol+1, departure_day, departure_month, now.year, parsed_json['soluzioni'][sol]['durata'], risp)

            keyboard=buttons.searchResultMenuButtons(chat_id, trains, departure_time_keyboard, arrival_time_keyboard, departure, arrival)
            bot.sendMessage(chat_id, emojize(response, use_aliases=True), parse_mode='html', disable_web_page_preview=None, disable_notification=None, reply_markup=keyboard)
    except Exception as e: 
        print(e)
        keyboard=buttons.backMainMenuButtons()
        bot.sendMessage(chat_id, emojize("Le API di viaggiotreno non sono in grado di rispondere a questa richeiesta anche se il tuo comando e' valido.\n\nIl motivo e' sconosciuto e da attributire a viaggiatreno.it\n\nCerca qui il tuo treno: http://www.trenitalia.com/ \n\nQuando sai il numero del tuo treno torna qui!", use_aliases=True), parse_mode='html', disable_web_page_preview=None, disable_notification=None, reply_markup=keyboard)
...
def date_parser (command):
    ...
    if(int(mese)>12):
        return ("Attenzione, il Mese inserito e' inesistente.\n\\nMese ricevuto = %s\n\nVerra' utilizzato il MESE ODIERNO" %(mese), "")
