...
#-----  preventive check before departure      
        if(start_time_difference - 15>=0 and start_time_difference - 15 <= 2): #If the train leaves in 15 minutes, check if the train IS REGULAR OR NOT       
            requestedTrain=databaseController.createTrain(row['train_number']) #create the object Train by the number in DB
            if row['last_alert'] != (requestedTrain.state.lower()+requestedTrain.alert.lower()): #IF THE TRAIN IS NOT REGULAR I ALERT THE USER THAT THE STATE IS CHANGED  
                keyboard=buttons.alertMenuButtons(requestedTrain.number)
                bot.sendMessage(row['user_id'], emojize(':warning:<b>ATTENZIONE</b>:warning:\nil Treno %s delle: %s:%s\n<b>Direzione:</b> %s\n\n<i>Risulta</i> <b>%s</b>\n\n<b>Ritardo:</b> %s\n\n%s'%(requestedTrain.number, requestedTrain.departure_datetime.hour, requestedTrain.departure_datetime.minute, requestedTrain.destination, requestedTrain.state, requestedTrain.delay, requestedTrain.alert), use_aliases=True), parse_mode='html', disable_web_page_preview=True, disable_notification=None, reply_markup=keyboard) #invio il messaggio
                try:
                    cursor.execute("UPDATE user_train SET last_alert='%s%s' WHERE user_id='%s' AND train_id='%s';" %(requestedTrain.state.lower(), requestedTrain.alert.lower(), row['user_id'], row['train_id'])) #Use REPLACE instead of INSERT for update old records if exists
                    database.commit()
...
#----- pre-departure message
        if(start_time_difference - 5>=0 and start_time_difference - 5 <= 2): #If the train leaves in 5 minutes, SEND THE DEPARTINGMSG TO USER        
            ...
            try:
                ...
                msg=requestedTrain.departingMsg(row['origin'], row['destination']) #if its all ok i call the method of the Train object for create the message for departing trains
            try:
                keyboard=buttons.alertMenuButtons(requestedTrain.number)
...
#----- if while is running is not regular
            if row['last_alert'].lower() != (requestedTrain.state.lower()+requestedTrain.alert.lower()): #IF THE TRAIN IS NOT REGULAR I ALERT THE USER THAT THE STATE IS CHANGED 
                keyboard=buttons.alertMenuButtons(requestedTrain.number)
                bot.sendMessage(row['user_id'], emojize(':warning:<b>ATTENZIONE</b>:warning:\nil Treno %s delle: %s:%s\n<b>Direzione:</b> %s\n\n<i>Risulta</i> <b>%s</b>\n\n<b>Ritardo:</b> %s\n\n%s'%(requestedTrain.number, requestedTrain.departure_datetime.hour, requestedTrain.departure_datetime.minute, requestedTrain.destination, requestedTrain.state, requestedTrain.delay, requestedTrain.alert), use_aliases=True), parse_mode='html', disable_web_page_preview=True, disable_notification=None, reply_markup=keyboard) #invio il messaggio
                try:
                    cursor.execute("UPDATE user_train SET last_alert='%s%s' WHERE user_id='%s' AND train_id='%s';" %(requestedTrain.state.lower(), requestedTrain.alert.lower(), row['user_id'], row['train_id'])) #Use REPLACE instead of INSERT for update old records if exists
...
#----- if the train is halfway for the user trip i update the user
            ...
            middle_time_difference=((middle_time_difference - now).total_seconds()/60.0) #difference (in minute) by now
            if(middle_time_difference>=0 and middle_time_difference<= 2): #If the train is halfway (for the user trip)      
                try:
                    if 'Error' in requestedTrain:
                        msg=requestedTrain
                except:
                    msg=requestedTrain.middleMsg() #if its all ok i call the method of the Train object for create the message for departing trains
