...
class Train:
    ...
    def __init__(self, id, number, origin, destination, departure_datetime, arrival_datetime, duration, delay, state, last_detection_time, last_detection_station, stations, alert, last_update):
        self.id = id
        self.number = number
        self.origin = origin
    ...
    def realTimeMsg(self):
        ...
        if '--' in self.last_detection_station:
            msg="\t\t:bullettrain_front: <i>Treno %s</i>\n%s :arrow_right: %s\n\n<b>Ora - Bin - Stazione</b>%s\n\n:clock3: <b>Durata</b>: %s ore\n:loudspeaker: <b>Stato</b>: %s\n\n:satellite:<b>Treno NON ancora partito da %s</b>\n\n%s" %(self.number, self.origin, self.destination, stations, self.duration, self.state, self.origin, self.alert)
        else:
            msg="\t\t:bullettrain_front: <i>Treno %s</i>\n%s :arrow_right: %s\n\n<b>Ora - Bin - Stazione</b>%s\n\n:clock3: <b>Durata</b>: %s ore\n:loudspeaker: <b>Stato</b>: %s\n\n%s\n\n:satellite:<b>Ultimo rilevamento</b> %s\n:earth_africa: Presso <b>%s</b>\n\n%s" %(self.number, self.origin, self.destination, stations, self.duration, self.state, delayMsg, self.last_detection_time, self.last_detection_station, self.alert)
        return msg
    ...
    def departingMsg(self, origin, destination):
        ...
        if '--' in self.last_detection_station:
            msg=":bullettrain_front:Il tuo <i>Treno %s</i> dovrebbe partire <b>tra 5 minuti!</b> :running:\n\n%s :arrow_right: %s\n\n<b>Ora - Bin - Stazione</b>%s\n\n:clock3: <b>Durata</b>: %s ore\n:loudspeaker: <b>Stato</b>: %s\n\n:satellite:<b>Treno NON ancora partito da %s</b>\n\n%s" %(self.number, origin, destination, stations, self.duration, self.state, self.origin, self.alert)
        else:
            msg=":bullettrain_front:Il tuo <i>Treno %s</i> dovrebbe partire <b>tra 5 minuti!</b> :running:\n\n%s :arrow_right: %s\n\n<b>Ora - Bin - Stazione</b>%s\n\n:clock3: <b>Durata</b>: %s ore\n:loudspeaker: <b>Stato</b>: %s\n\n%s\n\n:satellite:<b>Ultimo rilevamento</b> %s\n:earth_africa: Presso <b>%s</b>\n\n%s" %(self.number, origin, destination, stations, self.duration, self.state, delayMsg, self.last_detection_time, self.last_detection_station, self.alert)
        return msg
    ...
    def middleMsg(self):
        ...
        if '--' in self.last_detection_station:
            msg=":bullettrain_front:<i>Treno %s</i> direzione %s\n\nDovresti essere a meta' viaggio!\n\n<b>Stato</b>: %s\n\n:satellite:<b>Treno NON ancora partito da %s</b>\n\n%s" %(self.number, self.destination, self.state, self.origin, self.alert)
        else:
            msg=":bullettrain_front:<i>Treno %s</i> direzione %s\n\nDovresti essere a meta' viaggio!\n\n<b>Stato</b>: %s\n\n%s\n\n:satellite:<b>Ultimo rilevamento</b> %s\n:earth_africa: Presso <b>%s</b>\n\n%s" %(self.number, self.destination, self.state, delayMsg, self.last_detection_time, self.last_detection_station, self.alert)
        return msg
    ...
    def stationsParser(self):
        ...
        for cont in range(0,nf):
            oraleggibile=dt.datetime.fromtimestamp(float(parsed_json[cont]['programmata']//1000.))
            ...
            if self.last_detection_station in stationName:
                ...
                stations+=('\n%s - <b>%s</b> - %s:round_pushpin:' %(oraleggibile.strftime('%H:%M'), track, stationName))
            else:
                stationName=stationName[:20].title()
                stations+=('\n%s - <b>%s</b> - %s' %(oraleggibile.strftime('%H:%M'), track, stationName))
