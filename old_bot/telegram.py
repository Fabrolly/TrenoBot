        ...
        if isinstance(response, basestring):
            if(response is not None):
                if keyboard is not None:
                    ...
                    bot.sendMessage(chatId, emojize(response, use_aliases=True), parse_mode='html', disable_web_page_preview=None, disable_notification=None)
            else:
                print ('messaggio non valido')
        else:
            for item, key in zip(response, keyboard):
                if(response is not None):
                    bot.sendMessage(chatId, emojize(item, use_aliases=True), parse_mode='html', disable_web_page_preview=None, disable_notification=None, reply_markup=key)
                else:
                    print ('messaggio non valido')
