...
def createTrain(trainNumber):
    ...
    row=cursor.execute("SELECT * FROM trains WHERE number = '%d'" %(int(trainNumber)))
    dbLine=cursor.fetchone()

    if(row==1 and ((now - dbLine['last_update']).total_seconds()/60.0) <1.5): # if the train is in DB and last update recent than 1,5 minutes (1:30 min)
...
def addOrUpdateTrainDB(number):
    ...
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S") #Actual time, for the last_update filed
    fermate=raw_jsonapi[raw_jsonapi.index('"fermate":')+10:raw_jsonapi.index('}],')+2] #list of all the stops, with binary and other info. I leave it in JSON.

    #Prepare string for the content filed in INSERT record on DB
    content_filed1=("'%s', %s, '%s', '%s', '%s', '%s', '%s',"%(idTrain, jsonapi['numeroTreno'], jsonapi['origineZero'], jsonapi['destinazioneZero'], convert_timestamp(jsonapi['orarioPartenza']), convert_timestamp(jsonapi['orarioArrivo']), jsonapi['compDurata']))
    content_filed2=("%s, '%s', '%s', '%s', '%s', '%s', '%s'"%(jsonapi['ritardo'], state, convert_timestamp(jsonapi['oraUltimoRilevamento']), jsonapi['stazioneUltimoRilevamento'], fermate, jsonapi['subTitle'], now))
    content_filed_db='%s %s'%(content_filed1, content_filed2)

    #Insert the record into database "TRENOBOT", table "train"
    try:
        cursor.execute("USE TRENOBOT;")
        cursor.execute("REPLACE INTO trains (%s) VALUES (%s);" %(field_train_db, content_filed_db)) #Use REPLACE instead of INSERT for update old records if exists
        database.commit()
        database.close()
...
def trainObjFromDb(dbLine): #create the Train object from the DB
    usedTrain=train.Train(dbLine['id'], dbLine['number'], dbLine['origin'], dbLine['destination'], dbLine['departure_datetime'], dbLine['arrival_datetime'], dbLine['duration'], dbLine['delay'], dbLine['state'], dbLine['last_detection_time'], dbLine['last_detection_station'], dbLine['stations'], dbLine['alert'], dbLine['last_update'])
    return usedTrain
